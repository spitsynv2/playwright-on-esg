...
4. The test sends `DELETE /session/<sessionId>` to end the session.

### Refresh model

The refresh request replaces the browser inside the same session. The session and its artifacts
...
- Send the next refresh with the original ID or with the latest child ID. Both reach the same session.
...
The remote session exposes a clipboard endpoint under the session:
...
- `ESG_BROWSER_CPU` — Session CPU units. Playwright uses a minimum of 1024.
- `ESG_BROWSER_MEMORY` — Session memory in MB. Playwright uses a minimum of 2048.
...
- `TEST_TIMEOUT_MS` is the Playwright timeout. It bounds each test and each hook.
...
- To run refresh scenarios in parallel, add one serial group for each parallel session, then
  set `WORKERS` to the group count.
